...
const Invitation = ({user}: IProps) => {
    ...
    const toggleModalReset = () => setModalResetPasswordOpen(!modalResetPasswordOpen)
    ...
    return (
        <Social>
            ...
            <div className="flex items-center gap-x-2 bg-[#BBD5C1] px-3 py-1 rounded justify-center mb-5">
                ...
                <p className="text-green-700 font-semibold text-sm">Invitation</p>
            </div>
            <div>
                <h1 className="text-center font-bold text-3xl uppercase">Halo, {user.name}</h1>
            ...
            </Button>
            {
                modalResetPasswordOpen && (
                    <ResetPasswordModal onClose={toggleModalReset} userId={user.id} />
                )
            }
        </Social>
    )
