...
export const AvatarForm = () => {
    ...
    const handleChange = async (e: ChangeEvent<HTMLInputElement>) => {
        ...
        toast.promise(
            axios.post(route("user.avatar.update", user.id), {avatar: uploadedFile}, {
                headers: {
    ...
    return (
        <div className="flex flex-col">
            <div
                className="w-[140px] h-[140px] bg-white rounded-full flex items-center justify-center overflow-hidden border-2">
                {
            ...
            <Button disabled={isLoading} onClick={handleClick} variant="outline" className="mt-3 text-xs w-max ml-5">
                Upload foto
            </Button>
        </div>
    )
