...
export const CreateFormAssessment = ({year, semester}: IProps) => {
    ...
    const [selectedStep, setSelectedStep] = useState<"selectTeacher" | "selectDate">("selectTeacher");
                ...
                <div className="px-6 py-4 max-h-[600px] w-full min-h-[600px] font-albert">
...

                    {
                        selectedStep === "selectTeacher" && (
                            <div className="w-full flex flex-col mt-4 h-[450px] overflow-y-auto">
                                <Heading title={"Guru"} description="Silahkan pilih guru"/>
                                {isLoading && (
                                            ...
                                            {teachers.map((teacher) => (
                                                ...
                                                >
                                                    <Avatar className="w-8 h-8">
                                                        <AvatarImage src={ziggy?.url + "/storage/" + teacher?.profile_picture}/>
                                                        <AvatarFallback>
                                                            {
...

                    {
                        selectedStep === "selectDate" && (
                            <div>
                                <Heading title="Tanggal" description="Silahkan pilih tanggal pelaksanaan supervisi" />
                                <div className="flex flex-col gap-y-4">
                                    <div className="mt-6 flex sm:flex-row xs:flex-col gap-x-3">
                                        <div className="w-1/2">
                                            <Label htmlFor="date_start">Tanggal mulai</Label>
                                            <Input type="date" id="date_start" name="date_start" onChange={handleChange}/>
                                        </div>
                                        <div className="w-1/2">
                                            <Label htmlFor="time_start">Waktu mulai</Label>
                                            <Input type="time" id="time_start" name="time_start" onChange={handleChange}/>
                                        </div>
                                    </div>
                                    <div className="mt-6 flex sm:flex-row xs:flex-col gap-x-3">
                                        <div className="w-1/2">
                                            <Label htmlFor="date_end">Tanggal berakhir</Label>
                                            <Input type="date" id="date_end" onChange={handleChange} name="date_end"/>
                                        </div>
                                        <div className="w-1/2">
                                            <Label htmlFor="time_end">Waktu berakhir</Label>
                                            <Input type="time" id="time_end" name="time_end" onChange={handleChange}/>
                                        </div>
                                    </div>
                                </div>
                            </div>
